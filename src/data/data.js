const DATA = {
    // BOOKS SECTION
    "books": [
        {
            "title": "Radical Candor",
            "link": "https://www.amazon.com/Radical-Candor-Kim-Scott/dp/B01KTIEFEE",
            "image": "src/images/books/book-1.png",
            "image2x": "src/images/books/book-1@2x.png",
            "image3x": "",
        },
        {
            "title": "Hacking Growth",
            "link": "https://www.amazon.com/Hacking-Growth-Fastest-Growing-Companies-Breakout-ebook/dp/B01IZTCF6Y",
            "image": "src/images/books/book-2.png",
            "image2x": "src/images/books/book-2@2x.png",
        },
        {
            "title": "Measure What Matters",
            "link": "https://www.amazon.com/Measure-What-Matters-Google-Foundation-ebook/dp/B078FZ9SYB",
            "image": "src/images/books/book-3.png",
            "image2x": "src/images/books/book-3@2x.png",
        },
        {
            "title": "The Stranger",
            "link": "https://www.amazon.com/Stranger-Vintage-International-Albert-Camus-ebook/dp/B008QLXSG8",
            "image": "src/images/books/book-4.png",
            "image2x": "src/images/books/book-4@2x.png",
        },
        {
            "title": "Who Owns the Future?",
            "link": "https://www.amazon.com/Who-Owns-Future-Jaron-Lanier-ebook/dp/B008J2AEY8",
            "image": "src/images/books/book-5.png",
            "image2x": "src/images/books/book-5@2x.png",
        },
        {
            "title": "Traction",
            "link": "https://www.amazon.com/Traction-Get-Grip-Your-Business-ebook/dp/B007QWLLV2",
            "image": "src/images/books/book-6.png",
            "image2x": "src/images/books/book-6@2x.png",
        },
        {
            "title": "Steal Like an Artist",
            "link": "https://www.amazon.com/Steal-Like-Artist-Things-Creative-ebook/dp/B0074QGGK6",
            "image": "src/images/books/book-7.png",
            "image2x": "src/images/books/book-7@2x.png",
        },
        {
            "title": "Todo es Personal",
            "link": "https://www.amazon.com/Todo-Personal-Relaciones-Alcanzar-Spanish-ebook/dp/B00JLL1S1E",
            "image": "src/images/books/book-8.png",
            "image2x": "src/images/books/book-8@2x.png",
        },
    ],

    // MUSIC SECTION
    "music": [
        {
            "title": "",
            "link": "https://www.spotify.com",
            "image": "./src/images/music/music-1.png",
            "image2x": "./src/images/music/music-1@2x.png",
            "image3x": "",
        },
        {
            "title": "",
            "link": "https://www.spotify.com",
            "image": "src/images/music/music-2.png",
            "image2x": "src/images/music/music-2@2x.png",
        },
        {
            "title": "",
            "link": "https://www.spotify.com",
            "image": "src/images/music/music-3.png",
            "image2x": "src/images/music/music-3@2x.png",
        },
        {
            "title": "",
            "link": "https://www.spotify.com",
            "image": "src/images/music/music-4.png",
            "image2x": "src/images/music/music-4@2x.png",
        },
        {
            "title": "",
            "link": "https://www.spotify.com",
            "image": "src/images/music/music-5.png",
            "image2x": "src/images/music/music-5@2x.png",
        },
        {
            "title": "",
            "link": "https://www.spotify.com",
            "image": "src/images/music/music-6.png",
            "image2x": "src/images/music/music-6@2x.png",
        },
        {
            "title": "",
            "link": "https://www.spotify.com",
            "image": "src/images/music/music-7.png",
            "image2x": "src/images/music/music-7@2x.png",
        },
        {
            "title": "",
            "link": "https://www.spotify.com",
            "image": "src/images/music/music-8.png",
            "image2x": "src/images/music/music-8@2x.png",
        },
        {
            "title": "",
            "link": "https://www.spotify.com",
            "image": "src/images/music/music-9.png",
            "image2x": "src/images/music/music-9@2x.png",
        },
    ],

    // ABOUT ME SECTION
    "about_me": [
        "I’m a designer, co-founder and CEO of ++hellohello, a human-centric design agency shipping products and experiences for companies all over the world.",

        "After 15 years working in all kinds of digital agencies, freelancing and working remotely, I now dedicate my time to build a common place for design, technology, strategy, innovation and creativity.",

        "Prior to ++hellohello I co-founded and worked as Product Designer of Oincs, a tool to better serve the community around traffic and safety, designed to take care of people and keep them alert of everything that's happening on the streets, all thanks to the cooperation of users who share and update information in real time to the benefit of all, through a mobile app.",

        "With more than 150K registered users in less than a year, organically and without the help of investors, Oincs was declared a sponsor brand of the country and the number one information platform for the major news media in Uruguay, sharing our data on TV news, radios and online newspapers. In addition to Uruguay, Oincs reached 11 countries in the region, positioning itself in the top 3 spots in the app stores rankings of downloads.",

        "### How can I help?",

        "I love helping designers, engineers, and founders in any way I can. I like to do this in two ways:",

        "1. The email in this website is a good place to start and my Twitter DMs are open. Let me know how I can help, ideally with specific questions or problems you're working through. I can be helpful in thinking about job interviews, portfolio presentations, design work, working at startups vs. large companies, building side projects. If I don't reply right away, please follow up – sometimes things slip through the cracks! But I'll do my best to reply quickly.",

        "2. If you find yourself in Montevideo, I'd love to meet you for coffee. Just shoot me an email or DM on Twitter and we can find a time. If you have specific questions or problems you’d like to talk about the better. Of course, I won't always have answers, but I think I can be a helpful listener and bounce questions back at you to hopefully provide logical next steps to pursue. If you’d rather just meet and chat casually, that’s fine too!",
    ]
}